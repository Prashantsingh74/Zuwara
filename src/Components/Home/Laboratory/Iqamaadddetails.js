...
import Iqamaaddpatient from './Iqamaaddpatient';
...
function Iqamaadddetails() {
    const navigate = useNavigate();
    const { IqamaPatientName,userDetails, username, signupFormData, isAuthenticated } = useContext(Context);
    ...
    const handleSearch = (event) => {
        ...
        if (term.trim() === '') {
            ...
            if (healthcareName.includes(term) || serviceName.includes(term)) {
                setFilteredResults([{
                    Healthcare: appointmentData.HealthcarName,
                    Servicename: appointmentData.Servicename
                }]);
            } else {
                setFilteredResults([]);
...
export default Iqamaadddetails;